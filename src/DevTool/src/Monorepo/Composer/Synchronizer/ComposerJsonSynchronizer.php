...
final class ComposerJsonSynchronizer
{
    ...
    public function __construct(ComposerJson $rootComposer)
    {
        ...
        $this->monorepoDir = null === $rootComposer->getFilename() ? '' : dirname($rootComposer->getFilename()) . '/';
    ...
    {
        if (1 < count($this->branchAlias) || isset($this->branchAlias['null'])) {
            $options = [];
    ...
    {
        \assert(null !== $composerJson->getFilename());
